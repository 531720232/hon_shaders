...
VS_OUTPUT VS( VS_INPUT In )
{
	...
	float fPositionOff = dot(normalize(vPosition2.xyz), 1.0f) * 0.25f;
	float fXT = (fPositionOff * PI);
	float fPhaseBrushOffset = (vPosition.z * 0.002f) * (cos((fTime + fXT) * vFoliage.z) * vFoliage.w);
	vPosition2.x += fPhaseBrushOffset * 0.5f;
	vPosition2.y += fPhaseBrushOffset * 0.3f;
