...
float	ShadowedVSM(float4 TexcoordLight)
{
	float2 moments = tex2Dproj(shadowmap, TexcoordLight);
	float depth = TexcoordLight.z / TexcoordLight.w;
	float e_x2 = moments.y;
	float ex_2 = moments.x * moments.x;
	float variance = e_x2 - ex_2;
	float mD_2 = moments.x - depth;
	mD_2 = mD_2 * mD_2;
	float p = variance / (variance + mD_2);

	float shadow = max(p, depth <= moments.x);
...
#else
	return saturate(ShadowedVSM(TexcoordLight));
#endif
}